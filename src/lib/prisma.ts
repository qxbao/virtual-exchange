import { PrismaClient, Prisma, Order, OrderStatus, User, MarketData, Position, Trade } from '@prisma/client'
...
const socketExtension = Prisma.defineExtension({
    ...
    query: {
        $allModels: {
            ...
            async $allOperations({ __internalParams: { transaction }, args, query, model, operation }) {
                ...
                try {
                    switch (model) {
                      case 'Order':
                        await handleOrderChanges(operation, result as Order, orderPrevStat);
                        break;
                        
                      case 'Position':
                        await handlePositionChanges(operation, result as Position);
                        break;
                        
                      case 'MarketData':
                        await handleMarketDataChanges(operation, result as MarketData);
                        break;
                        
                      case 'User':
                        if (operation === 'update' && args?.data?.balance !== undefined) {
                    ...
                    console.error(`Error in Prisma extension for ${model}.${operation}:`, error);
                  }
...
async function handleOrderChanges(operation: string, data: Order, orderPrevStat: OrderStatus | null) {
